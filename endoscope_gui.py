...
class EndoscopeGUI(ctk.CTk):
    ...
    def create_ui(self):
        ...
        self.sidebar = ctk.CTkFrame(
            self, width=420, corner_radius=0,
        ...
        ctk.CTkLabel(header_frame, text="SMART ENDOSCOPE",
                     font=ctk.CTkFont(size=24, weight="bold"),
                     text_color=self.theme["text_main"]).pack(anchor="w")
        ctk.CTkLabel(header_frame, text="SBE 3220 Clinical Simulator",
                     font=ctk.CTkFont(size=14),
                     text_color=self.theme["accent_hover"]).pack(anchor="w")

        # Top Buttons Card
        btn_card = self.create_ui_card(self.sidebar)
        ctk.CTkButton(
            btn_card, text="📁 Load Patient Data", command=self.load_image,
            height=45, fg_color=self.theme["accent"],
            hover_color=self.theme["accent_hover"],
            font=ctk.CTkFont(weight="bold"),
        ).pack(pady=(0, 10), fill="x")

        ctk.CTkButton(
            btn_card, text="📷 Capture High-Res Image", command=self.capture_image,
            height=45, fg_color="transparent", border_width=2,
            border_color=self.theme["accent"], hover_color="#1b222b",
        ...
        ctk.CTkButton(
            quick_actions, text="↺ Reset Parameters", command=self.reset_features,
            height=35, fg_color=self.theme["border"], hover_color=self.theme["danger"],
        ).pack(side="left", expand=True, padx=(0, 5), fill="x")

        self.btn_hud = ctk.CTkButton(
            quick_actions, text="👁 Hide HUD", command=self.toggle_hud,
            height=35, fg_color=self.theme["border"], hover_color=self.theme["accent_hover"],
        )
        ...
        self._switches = {}
        for attr, label in (
            ("nbi",     "Narrow Band Imaging (NBI)"),
            ("fisheye", "Lens Distortion (Fisheye)"),
            ("pip",     "Raw Hardware Feed (PiP)"),
        ):
            sw = ctk.CTkSwitch(optics_card, text=label, command=self.update_toggles,
                               progress_color=self.theme["accent"])
            sw.pack(pady=(0, 10), anchor="w")
            self._switches[attr] = sw

        # Parametric DSP Card
        dsp_card = self.create_ui_card(self.sidebar, title="PARAMETRIC DSP CONTROL")

        # (name, from_, to, default, callback, fmt, reverse)
        slider_specs = [
            ("light",   "Light Intensity",             0.1,  3.0,  1.0,  self.update_illumination, "{:.1f}x", False),
            ("denoise", "Edge-Preserving Denoise",     0,    150,  0,    self.update_denoise,       "{:.0f}",  False),
            ("clahe",   "CLAHE Contrast Limit",        0.0,  5.0,  0.0,  self.update_clahe,         "{:.1f}",  False),
            ("texture", "Sobel Texture Overlay",       0.0,  1.0,  0.0,  self.update_texture,       "{:.2f}",  False),
            ("anomaly", "Anomaly Sensitivity (px)",    100,  5000, 5000, self.update_anomaly,       "{:.0f}",  True),
        ]
        self._sliders = {}
        self._slider_labels = {}
        for name, text, from_, to, default, cb, fmt, reverse in slider_specs:
            sl, lbl = self.create_advanced_slider(dsp_card, text, from_, to, default, cb, fmt, reverse)
            self._sliders[name] = (sl, default, to if reverse else default, fmt)
            self._slider_labels[name] = lbl

        # Instructions Footer
        nav_text = "Navigation Instructions:\n• Click & Drag to steer the insertion tube.\n• Mouse Wheel to advance/retract (Zoom)."
        ctk.CTkLabel(self.sidebar, text=nav_text,
                     text_color=self.theme["text_dim"], justify="left").pack(
            side="bottom", pady=20, padx=20, anchor="w")
    ...
    def create_ui_card(self, parent, title=None):
        ...
        if title:
            ctk.CTkLabel(inner, text=title,
                         ...
                         text_color=self.theme["text_dim"]).pack(anchor="w", pady=(0, 10))
        return inner

    def create_advanced_slider(self, parent, text, from_, to, default, command,
                               format_str="{:.2f}", reverse=False):
        ...
        value_label = ctk.CTkLabel(header, text=format_str.format(default),
    ...
    def reset_features(self):
        ...
        for name, (sl, default, init_val, fmt) in self._sliders.items():
            sl.set(init_val)
            self._slider_labels[name].configure(text=fmt.format(init_val))

    def toggle_hud(self):
        """Toggles the telemetry overlay and updates button text."""
        self.engine.enable_hud = not self.engine.enable_hud
        self.btn_hud.configure(text="👁 Hide HUD" if self.engine.enable_hud else "👁 Show HUD")
    ...
    def update_toggles(self):
        self.engine.enable_nbi     = bool(self._switches["nbi"].get())
        self.engine.enable_fisheye = bool(self._switches["fisheye"].get())
        self.engine.enable_pip     = bool(self._switches["pip"].get())
